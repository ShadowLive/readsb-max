...
static struct argp_option optionsReadsb[] = {
    ...
    {"rssi-gate-multibit", OptRssiGateMultibit, "<dBFS>", 0, "Min RSSI for multi-bit CRC correction (e.g. -40 = -40 dBFS, 0 = disabled)", 1},
    {"metric", OptMetric, 0, 0, "Use metric units", 1},
    {"show-only", OptShowOnly, "<addr>", 0, "Show only messages by given ICAO on stdout", 1},
    ...
    {"aggressive", OptAggressive, 0, OPTION_HIDDEN, "Deprecated: 4-bit CRC is now default", 1},
    ...
    {"all-phases", OptAllPhases, 0, 0, "Test all phases unconditionally (more CPU, may find a few more messages)", 1},
    ...
    {"icao-extend-ttl", OptIcaoExtendTtl, 0, 0, "Extend ICAO filter TTL 10x for file processing (600 sec instead of 60)", 1},
    {"mode-s-strict", OptModeSStrict, 0, 0, "Only accept Mode-S messages if aircraft was recently seen via ADS-B", 1},
    {"no-icao-fix-modes", OptNoIcaoFixModeS, 0, 0, "Disable ICAO correction for Mode-S (DF0/4/5/16/20/21) - prevents wrong ICAO derivation", 1},
    {"validate-icao-correction", OptValidateIcaoCorrection, 0, 0, "Validate ICAO-corrected Mode-S messages by checking altitude/squawk against tracked aircraft", 1},
    {"fir-filter", OptFirFilter, "<2-3>", 0, "Apply magnitude pre-filter (2=gentle edge, 3=contrast)", 1},
    {"fir-slice", OptFirSlice, "<1-2>", 0, "Use alternative slice coefficients (1=FIR tuned, 2=optimal)", 1},
    {"collision-detect", OptCollisionDetect, 0, 0, "Detect and recover colliding messages (experimental, more CPU)", 1},
